...
class MapLibreMapController extends ChangeNotifier {
  ...
  Future<void> _updateMapOptions(Map<String, dynamic> optionsUpdate) async {
    try {
      ...
      // Error handling: void method, catch block left empty
    }
  }
  ...
  Future<void> updateMyLocationTrackingMode(
    ...
    try {
      return await _maplibrePlatform
  ...
  Future<Symbol> addSymbol(SymbolOptions options, [Map? data]) async {
    ...
    if (symbolManager == null) return symbol;
  ...
  Future<List<Symbol>> addSymbols(List<SymbolOptions> options,
    ...
    if (symbolManager == null) return symbols;
  ...
  Future<Line> addLine(LineOptions options, [Map? data]) async {
    ...
    if (lineManager == null) return line;
  ...
  Future<List<Line>> addLines(List<LineOptions> options,
    ...
    if (lineManager == null) return lines;
  ...
  Future<void> updateCircle(Circle circle, CircleOptions changes) async {
    // Verify circleManager exists
    if (circleManager == null) return;

    // Verify circle and its options are not null before accessing
    if (circle.options == null) return;
  ...
  Future<Fill> addFill(FillOptions options, [Map? data]) async {
    ...
    if (fillManager == null) return fill;
  ...
  Future<List<Fill>> addFills(List<FillOptions> options,
    ...
    if (fillManager == null) return fills;
  ...
  Future<void> addImage(String name, Uint8List bytes, [bool sdf = false]) async {
    try {
      await _maplibrePlatform.addImage(name, bytes, sdf);
    } catch (e) {
      debugPrint('MAPLIBRE => addImage => $e');
      return Future.value();
    }
  }
